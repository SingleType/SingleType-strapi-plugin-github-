...
const Repo = () => {
    ...
    const createProject = (repo) => {
        ...
        .then((response) => {setRepos(repos.map((item) => 
          item.id !== repo.id
           ? item 
           : {
              ...item,
          ...
          showAlert({
              title: "Project created",
               message: `Succussfully created project ${response.data.title}`,
               variant: "succes",
          });})
        .catch((error) => {
          showAlert({
    ...
    const deleteProject = (repo) => {
        ...
        showAlert({
            title: "Project deleted",
             message: `Succussfully deleted project ${response.data.title}`,
             variant: "succes",
        });
        }).catch((error) => {
          showAlert({
      ...
      axios.post("/github-projects/projects", {
        ...
        showAlert({
          title: "Projects created",
           message: `Succussfully created ${response.data.length} projects`,
           variant: "succes",
      });
      }).catch((error) => {
        showAlert({
      ...
      axios.delete("/github-projects/projects", {
        ...
        showAlert({
          title: "Projects deleted",
           message: `Succussfully deleted ${response.data.length} projects`,
           variant: "succes",
      });
      }).catch((errror) => {
        showAlert({
    ...
    return (
        <Box padding={8} background="neutral100">
            ...
            <BulkActions selectedRepos={selectedRepos.map((repoId) => 
          ...
          <Tbody>
              {repos.map((repo) => {
                  const { id, name, shortDescripstion, url, projectId } = repo;
                  return (
              <Tr key={id}>
                  <Td>
                    <BaseCheckbox
                  ...
                  <Td>
                    <Typography textColor="neutral800">{shortDescripstion}</Typography>
                  </Td>
                  <Td>
                  ...
                  <Td>
                    ...
                    <Flex>
                      ...
                      <Box paddingLeft={1}>
                        <IconButton onClick={() => setDeletingRepo(repo)}
                            label="Delete" 
                            noBorder 
                            icon={<Trash />} />
                      </Box>
